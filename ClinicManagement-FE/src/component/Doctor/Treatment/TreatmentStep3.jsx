...
const TreatmentStep3 = ({
  ...
  const getTreatmentService = async () => {
    try {
      const dataa = await DoctorApiService.getTreatmentService(
        treatmentDetail.appointment.id
      );
  ...
  const getDrugAllergy = async () => {
    try {
      const dataa = await DoctorApiService.getDrugAllergyByPatientId(
        treatmentDetail.appointment.patient.id
      );
      ...
      : [
          {
            ...
            unit: "",
            usageInstruction: "",
            appointment: { id: treatmentDetail.appointment.id },
          },
        ];
  });
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response1 = await DoctorApiService.treatAppointmentStep2(
        treatDetail
      );
      ...
      for (const service of filteredServiceList) {
        ...
        const response2 = await DoctorApiService.treatAppointmentServiceStep2(
          formData
        );
      }
      if (
        treatDetail.prescriptionType === "KDCV" ||
        treatDetail.prescriptionType === "KDTK"
      ) {
        for (const pres of prescript) {
          const response3 = await DoctorApiService.treatAppointmentPresciption(
            pres
          );
        }
      }
        ...
        <div className="col-md-12 mb-3">
            ...
            {examServiceList?.map((item, index) => (
              <div className="card mb-2 p-3 mt-2" key={index}>
                ...
                <div className="form-group" style={{ display: "flex" }}>
                  <label style={{ width: "110px", marginRight: "75px" }}>
                    Nhận xét:
                  </label>
                  <textarea
                    className="form-control"
          ...
          <div className="card mb-2 p-3 mt-2">
            ...
            <div form-group mb-2 style={{ display: "flex" }}>
              <label style={{ width: "200px" }} className="mb-2">
                Chọn hướng điều trị:
              </label>
              <select
                style={{ width: "300px" }}
