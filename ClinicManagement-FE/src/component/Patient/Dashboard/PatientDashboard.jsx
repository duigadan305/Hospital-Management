...
const PatientDashboard = () => {
    ...
    const fetchAppointment = async () => {
      ...
      try {
        if (patientData) {
          ...
          const data = await PatientApiService.getAllAppointment(
            appointmentRequest
          );
  ...
  const updatedApppointmentStatus = async (data, type) => {
    try {
      ...
      if (response.statusCode === 200) {
        ...
        const apdata = await PatientApiService.getAllAppointment(
          appointmentRequest
        );
  ...
  const appointmentColumns = [
    ...
    },
    {
      ...
      render: function (appointmentData) {
        return (
          <div style={{ textAlign: "left" }}>{appointmentData?.reason}</div>
        );
      },
    },
    {
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            {(appointmentData?.status == "Pending" ||
              ...
              <Button
                ...
                onClick={() => showConfirmCancel(appointmentData, "Cancel")}
              >
                Hủy
              </Button>
            )}
