    ...
    useEffect(() => {
      ...
      const columns = [
        ...
        },
        {
            ...
            dataIndex: 'payment',
            key: 'payment',
            render: (payment) => (payment === 'No' ? 'Chưa thanh toán' : 'Đã thanh toán'),
            width: '20%', // Điều chỉnh độ rộng cột
        },
        {
            title: 'Hành động',
            key: 'action',
            render: (record) => {
                return (
                    <Link 
                        to={`/appointment-invoice/${record.id}`}
                        state={{ from: `/patient-invoice-detail/${id}` }} 
                    >
                        <Button type="primary">Xem hóa đơn</Button>
                    </Link>
                );
            },
            width: '15%',
