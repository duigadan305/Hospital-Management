...
const PatientTab = () => {
  ...
  const handleSearch = async (values) => {
    ...
    const response = await AdminApiService.getPatientsByCondition(request);
    setData(response.patientList);
    console.log("docccc=>", response.patientList);
  ...
  const handleSave = async (values) => {
    ...
    const response = await PatientApiService.updatePatientInfo(request);
    if (response.statusCode === 200) {
      ...
      const response = await AdminApiService.getPatientsByCondition(
        patientRequest
      );
      setData(response.patientList);
    }
      ...
      <Table
        columns={[
          ...
          },
          {
            ...
            render: (text, record) =>
              record.healthInsuranceNumber ? record.healthInsuranceNumber : "",
          },
          {
            title: "Thao tác",
            key: "actions",
